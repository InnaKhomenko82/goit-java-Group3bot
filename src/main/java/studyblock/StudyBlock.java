...
public class StudyBlock {
    public void handleCallbackQuery(Update update, TelegramController telegramController) { //реакция на нажатие кнопок
        if (!update.getCallbackQuery().getData().isEmpty()) { //.getData().equals("Java")
            System.out.println("yes");
            long chatId = update.getCallbackQuery().getFrom().getId();
            String messageText = "Вопросы:";
    ...
    public void handleTextUpdate(Update update, TelegramController telegramController) { // обработка Go
        if (update.getCallbackQuery().getData().equals("Go")) {
            long chatId = update.getCallbackQuery().getFrom().getId();
        ...
        keyboard.setKeyboard(
                Collections.singletonList(
                        Arrays.asList(
                                ...
                                InlineKeyboardButton.builder().text(":arrow_forwards:next").callbackData("next").build()
                        )
                )
        );
        return keyboard;
